...
const AcceptedOffers = () => {
  ...
  const getAcceptedOffers = async () => {
    try {
      ...
      const { data } = await axiosInstance.get(
        "api/v1/order/get-accepted_offers/client"
      );
      if (data.success) {
  ...
  const filteredOffers = offers.filter((offer) => {
    switch (selectedTab) {
      case "Completed":
        return offer.clientSideOrderStatus === "completed";
      case "Processing":
        return offer.clientSideOrderStatus === "processing";
      case "Pending":
        return offer.clientSideOrderStatus === "pending";
      case "Cancelled":
        return offer.clientSideOrderStatus === "Cancel";
      case "Search":
            ...
            <div className="flex gap-8 items-center flex-wrap">
              {!loading &&
                ...
                filteredOffers?.map((order) => (
                  <Badge
                    key={order._id}
                    content={`${order?.clientSideOrderStatus}`}
                    className={`text-[15px] arimo bg-buttoncolor text-hoverblack font-bold select-none ${
                      order?.clientSideOrderStatus === "processing"
                        ? "bg-green-500"
                        : ""
                    }`}
                  >
